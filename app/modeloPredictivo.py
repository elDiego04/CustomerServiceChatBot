...
new_data = pd.DataFrame({'Age': ['Adult'], 'Size': ['L'], 'Weather': ['Cold'], 'Time': ['Night'], 'Sex_Gender': ['F'], 'Event': ['Marriage'], 'Satisfaction': [1] })
...
new_data_encoded = encoder.transform(new_data[['Age', 'Size', 'Weather', 'Time', 'Event']])

# Concatenar las variables codificadas con las características originales
new_data_encoded = np.hstack((new_data_encoded.toarray(), new_data.drop(['Age', 'Size', 'Weather', 'Time', 'Event'], axis=1)))
